...
- 解释：
  - \(\mathbb{E}_{x \sim p_{data}(x)}[\log D(x)]\): 判别器试图最大化对真实样本的正确分类概率。
  - \(\mathbb{E}_{z \sim p_z(z)}[\log(1 - D(G(z)))]\): 判别器试图最大化对生成样本的正确判断概率，而生成器试图最小化。
  - \(D(G(z))\) 接近1（即欺骗判别器），生成器 \(G\) 希望最小化 \(\log(1 - D(G(z)))\)，使生成样本接近真实分布。
