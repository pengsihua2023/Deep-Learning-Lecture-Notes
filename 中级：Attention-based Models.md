...
```
我将修改代码，添加注意力权重的可视化，使用一个热图来展示第一个样本的注意力权重矩阵，帮助直观理解Attention机制如何关注不同词之间的关系。代码仍基于IMDb数据集，并使用PyTorch实现简单的Scaled Dot-Product Attention。由于您要求结果可视化，我将生成一个热图，显示注意力权重。
...
- 如果需要更多可视化（例如多个样本的热图或不同类型的图表），请进一步说明！

如果您想要其他类型的可视化（例如柱状图展示权重分布）或调整数据集大小，请告诉我！
```
