## 数学基础知识：矩阵和张量之间的转换
在深度学习中，矩阵和张量之间的转换是非常常见的操作，因为矩阵是二维张量（2阶张量），而张量是更广义的多维数据结构（包括标量、向量、矩阵及更高维）。结合您之前的提问（涉及 `NumPy`、4x3 矩阵 `[[0 1 0], [1 0 1], [0 0 1], [1 1 0]]`），我将用中文简洁说明深度学习中矩阵与张量之间的转换，重点介绍 `NumPy` 及深度学习框架（如 `TensorFlow` 和 `PyTorch`）中的实现，并与您的上下文相关联。
