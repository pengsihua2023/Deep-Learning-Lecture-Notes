...
在 **Scaled Dot-Product Attention** 里，缩放因子 $\sqrt{d_k}$ 的来源是 **Key 向量的维度大小**。
...
如果不用缩放，点积 $q_i \cdot k_j$ 的值会随着维度 $d_k$ 增大而变大，导致 Softmax 输入过大 → 输出趋近于 one-hot → 梯度消失。

因此通过除以 $\sqrt{d_k}$ 来 **标准化分布的尺度**，让 Softmax 在数值上更稳定。
...
* 设输入隐藏层维度为 $d_{\text{model}}$；
* 使用 $h$ 个注意力头（multi-head）；
* 每个头的 $Q, K, V$ 维度通常设为：
...
* BERT-base：$d_{\text{model}} = 768$，$h = 12$
  → 每个头 $d_k = 64$，缩放因子 = $\sqrt{64} = 8$。
* GPT-2 small：$d_{\text{model}} = 768$，$h = 12$
  → 同样 $d_k = 64$。
