...
这个图展示的是**自注意力机制（Self-Attention）** 的核心计算流程。

### 1. 输入表示

* **X**：输入序列，可以是一段文本对应的词向量序列，也可以是图像的特征向量序列。
* 每一行通常代表一个元素（比如一个词的embedding）。

### 2. 线性变换生成 Q, K, V

* **θ(X) → Q (Query)**：通过线性变换生成 Query 矩阵。
* **φ(X) → K (Key)**：通过线性变换生成 Key 矩阵，并且后面会转置。
* **g(X) → V (Value)**：通过线性变换生成 Value 矩阵。

这三个矩阵是注意力机制的核心：

* **Q** 表示当前元素提出的问题（要关注什么）。
* **K** 表示元素的属性或索引（别人能提供什么信息）。
* **V** 表示实际携带的信息（要传递的内容）。

### 3. 计算注意力分数 (Attention Score)

* **Q × Kᵀ**：计算 Query 和 Key 的相似度。
* 得到的矩阵是注意力分数，表示输入序列中每个元素对其它元素的“关注程度”。
* 通常还会加上 **缩放 (scale)** 和 **softmax**，使得分数更稳定、可解释。


### 4. 加权求和

* 用注意力分数去加权 **V (Value)**。
* 每个元素的最终表示，是它从其它元素“借”来的信息的加权和。
* 这一步实现了“信息聚合”：序列中的每个位置，可以动态关注并整合其它位置的内容。
...
* **K**：别人能回答什么。
* **V**：别人真正提供的答案。
* **注意力分数 (QKᵀ)**：决定“我更信任谁的答案”。
* **输出 (Attention结果)**：我最终得到的答案，是所有人回答的加权平均。
